...
class wing_dataset(data.Dataset):
    def __init__(self, dataPath='wings', _set='train', transform = None, augment=None, predict=False):
        ...
        self.weights_path = self.root / 'weights.csv'
        self.weights = pd.read_csv(self.weights_path, index_col=0)['weight']
    ...
    def __getitem__(self, idx):
        ...
        return {'image': img, 'target': weight, 'fp': fp, 'fn': fn}
...
class WingWeightPredictionExperiment(pl.LightningModule):
    def __init__(self):
        super().__init__()
    ...
    def predict_step(self, batch, batch_idx):
        ...
        with open(csv_path, mode='a', newline='') as file:
            ...
            if write_header:
                writer.writerow(["Filename", "PredictedWeight"])
...
class WeightRegressionData(pl.LightningDataModule):
...
class PredictWeightRegressionData(pl.LightningDataModule):
    def __init__(self, dataPath: str = "path/to/dir"):
        super().__init__()
        self.dataPath = dataPath
...
if __name__ == "__main__":
    if args.checkpoint:
        experiment = WingWeightPredictionExperiment.load_from_checkpoint(args.checkpoint)
    else:
        experiment = WingWeightPredictionExperiment()
    ...
    if args.train:
        data = WeightRegressionData(args.dataPath)
